...
function handleFetch() {
  ...
  dialog.innerHTML = `
    ...
    <span style="margin-left: 8px;">Fetching code: ${code.value || 'empty'}</span>
  `
  dialog.open = true
...
function handleShare() {
  ...
  dialog.innerHTML = `
    ...
    <span style="margin-left: 8px;">Generate share code!</span>
  `
  dialog.open = true
  document.body.appendChild(dialog)
  setTimeout(() => dialog.remove(), 2000)
...
.code-input {
  width: 100%;
  margin-bottom: 16px;
  --mdui-text-field-input-text-align: center;
}
